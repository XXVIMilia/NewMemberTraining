 ...
 * @LastEditTime: 2021-02-25 15:42:05
...
static void ApplicationFunctionSet_SmartRobotCarLinearMotionControl(SmartRobotCarMotionControl direction, uint8_t directionRecord, uint8_t speed, uint8_t Kp, uint8_t UpperLimit)
{
  ...
  if (en != directionRecord || _millis() - is_time > 10)
  {
...
void ApplicationFunctionSet::ApplicationFunctionSet_SensorDataUpdate(void)
{
  { /*电压状态更新*/
    ...
    if (_millis() - VoltageData_time > 10) //10ms 采集并更新一次
    {
      VoltageData_time = _millis();
...
void ApplicationFunctionSet::ApplicationFunctionSet_RGB(void)
{
  ...
  {
    if ((_millis() - getAnalogue_time) > 3000)
    {
      getAnalogue_time = _millis();
    }
  }
  unsigned long temp = _millis() - getAnalogue_time;
...
void ApplicationFunctionSet::ApplicationFunctionSet_Tracking(void)
{
  ...
  if (Application_SmartRobotCarxxx0.Functional_Mode == TraceBased_mode)
  {
    ...
    {
      /*控制左右电机转动：实现匀速直行*/
      ApplicationFunctionSet_SmartRobotCarMotionControl(Forward, TraceBased_CarSpeed);
      timestamp = true;
      BlindDetection = true;
    }
    ...
    {
      /*控制左右电机转动：前右*/
      ApplicationFunctionSet_SmartRobotCarMotionControl(Right, TraceBased_CarSpeed);
      timestamp = true;
      BlindDetection = true;
    }
    ...
    {
      /*控制左右电机转动：前左*/
      ApplicationFunctionSet_SmartRobotCarMotionControl(Left, TraceBased_CarSpeed);
      timestamp = true;
      BlindDetection = true;
    }
    else //不在黑线上的时候。。。
    {
      ...
      /*Blind Detection*/
      if ((function_xxx((_millis() - MotorRL_time), 0, 200) || function_xxx((_millis() - MotorRL_time), 1600, 2000)) && BlindDetection == true)
      {
        ApplicationFunctionSet_SmartRobotCarMotionControl(Right, TraceBased_CarSpeed);
      }
      else if (((function_xxx((_millis() - MotorRL_time), 200, 1600))) && BlindDetection == true)
      {
        ApplicationFunctionSet_SmartRobotCarMotionControl(Left, TraceBased_CarSpeed);
      }
      else if ((function_xxx((_millis() - MotorRL_time), 3000, 3500))) // Blind Detection ...s ?
      {
    ...
    if (function_xxx(get_Distance, 0, 20))
    {
      ...
      for (int i = 1; i < 6; i += 2) //1、3、5 Omnidirectional detection of obstacle avoidance status
      {
        ...
        if (function_xxx(get_Distance, 0, 20))
        {
          ...
          if (5 == i)
          {
            ApplicationFunctionSet_SmartRobotCarMotionControl(Backward, ObstacleAvoidance_CarSpeed);
            _delay(500);
            ApplicationFunctionSet_SmartRobotCarMotionControl(Right, ObstacleAvoidance_CarSpeed);
            _delay(100);
            first_is = true;
            break;
          }
        ...
        else
        {
          ...
          case 1:
            ...
            break;
          case 3:
            ApplicationFunctionSet_SmartRobotCarMotionControl(Forward, ObstacleAvoidance_CarSpeed);
            break;
          case 5:
            ApplicationFunctionSet_SmartRobotCarMotionControl(Left, ObstacleAvoidance_CarSpeed);
            break;
          }
          _delay(100);
          first_is = true;
...
void ApplicationFunctionSet::ApplicationFunctionSet_Follow(void)
{
  ...
  if (Application_SmartRobotCarxxx0.Functional_Mode == Follow_mode)
  {
    ...
    if (false == function_xxx(ULTRASONIC_Get, 0, 20)) //前方 20 cm内无障碍物？
    {
      ...
      if (_millis() - time_Servo > 1000) //作用于舵机停留位置时长_1s
      {
        timestamp = 3;
        Position_Servo += 1;
        OneCycle += 1;
    ...
    else
    {
      ...
      if ((Position_Servo == 1))
      { /*控制左右电机转动：前进*/
        ApplicationFunctionSet_SmartRobotCarMotionControl(Forward, Follow_CarSpeed);
      }
      else if ((Position_Servo == 2))
      { /*控制左右电机转动：前右*/
        ApplicationFunctionSet_SmartRobotCarMotionControl(Right, Follow_CarSpeed);
      }
      else if ((Position_Servo == 3))
      {
        /*控制左右电机转动：前进*/
      ...
      { /*控制左右电机转动：前左*/
        ApplicationFunctionSet_SmartRobotCarMotionControl(Left, Follow_CarSpeed);
      }
    }
  }
  else
...
//     {
//       if ((_millis() - Application_SmartRobotCarxxx0.CMD_CarControl__millis) > (is_Timer)) //判断时间戳
//       {
//         CarControl_TE = true;
//         ApplicationFunctionSet_SmartRobotCarMotionControl(stop_it, 0);
...
//       CarControl_return = false;
//       CarControl = false;
//       Application_SmartRobotCarxxx0.CMD_CarControl__millis = 0;
//     }
//   }
// }
...
void ApplicationFunctionSet::CMD_CarControlTimeLimit_xxx0(void)
{
  ...
  if (Application_SmartRobotCarxxx0.Functional_Mode == CMD_CarControl_TimeLimit) //进入车子有时间限定控制模式
  {
    ...
    {
      if ((_millis() - Application_SmartRobotCarxxx0.CMD_CarControl_Millis) > (CMD_is_CarTimer)) //判断时间戳
      {
        CarControl_TE = true;
        ApplicationFunctionSet_SmartRobotCarMotionControl(stop_it, 0);
...
void ApplicationFunctionSet::CMD_LightingControlTimeLimit_xxx0(void)
{
  ...
  if (Application_SmartRobotCarxxx0.Functional_Mode == CMD_LightingControl_TimeLimit) //进入灯光有时间限定控制模式
  {
    ...
    {
      if ((_millis() - Application_SmartRobotCarxxx0.CMD_LightingControl_Millis) > (CMD_is_LightingTimer)) //判断时间戳
      {
        LightingControl_TE = true;
        FastLED.clear(true);
        Application_SmartRobotCarxxx0.Functional_Mode = CMD_Programming_mode; /*进入编程模式提示符<等待下一组控制命令的到来>*/
...
void ApplicationFunctionSet::ApplicationFunctionSet_IRrecv(void)
{
  ...
  if (true == IRrecv_en)
  {
    ...
    case /* constant-expression */ 13:
    {
      if (Application_SmartRobotCarxxx0.Functional_Mode == TraceBased_mode)
      {
        TraceBased_CarSpeed = 200;
      }
      else if (Application_SmartRobotCarxxx0.Functional_Mode == ObstacleAvoidance_mode)
      {
        ObstacleAvoidance_CarSpeed = 200;
      }
      else if (Application_SmartRobotCarxxx0.Functional_Mode == Follow_mode)
      {
        Follow_CarSpeed = 200;
    ...
    case /* constant-expression */ 14:
    {
      ...
      {
        if (ObstacleAvoidance_CarSpeed > 100)
        {
          ObstacleAvoidance_CarSpeed -= 5;
        }
      }
    ...
    if (IRrecv_button < 5)
    {
      ...
      if (_millis() - AppIRrecv.IR_PreMillis > 300)
      {
        IRrecv_en = false;
        Application_SmartRobotCarxxx0.Functional_Mode = Standby_mode;
        AppIRrecv.IR_PreMillis = _millis();
      }
    }
    else
    {
  ...
  if (c == '}') //数据帧尾部校验
  {
    ...
    else if (!error) //检查反序列化是否成功
    {
      ...
      case 2:                                                                     /*<命令：N 2> */
        ...
        Application_SmartRobotCarxxx0.CMD_CarControl_Millis = _millis();
#if _is_print
        //Serial.print('{' + CommandSerialNumber + "_ok}");
#endif
        break;
      ...
      case 7:                                                                          /*<命令：N 7> */
        ...
        Application_SmartRobotCarxxx0.CMD_LightingControl_Millis = _millis();
#if _is_print
        //Serial.print('{' + CommandSerialNumber + "_ok}");
#endif
        break;
